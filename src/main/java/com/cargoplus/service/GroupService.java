...
public final class GroupService {
    ...
    public GroupService(FileConfiguration config, CargoPlusColorConfig colors) {
        ...
        if (!isSafeGroupName(defaultGroup)) {
            throw new IllegalStateException("default-group invalido: " + defaultGroup);
        }

        for (String name : config.getStringList("hierarchy")) {
            String normalized = normalize(name);
            if (!isSafeGroupName(normalized)) {
                throw new IllegalStateException("Cargo invalido na hierarquia: " + name);
            }
            if (!hierarchy.add(normalized)) {
                throw new IllegalStateException("Cargo duplicado na hierarquia: " + normalized);
            }
        }
        if (hierarchy.isEmpty()) throw new IllegalStateException("hierarchy nao pode ser vazia.");
        if (!hierarchy.contains(defaultGroup)) {
            throw new IllegalStateException("default-group fora da hierarquia: " + defaultGroup);
        }
        ...
        if (section.getKeys(false).size() > MAX_GROUPS) {
            throw new IllegalStateException("Quantidade de cargos excede o limite de " + MAX_GROUPS + ".");
        }

        for (String rawName : section.getKeys(false)) {
            String name = normalize(rawName);
            if (!isSafeGroupName(name)) {
                throw new IllegalStateException("Nome de cargo invalido: " + rawName);
            }
            if (groups.containsKey(name)) {
                throw new IllegalStateException("Cargos duplicados apos normalizacao: " + name);
            }
            List<String> parents = normalizeGroups(section.getStringList(rawName + ".parents"));
            if (parents.size() > MAX_PARENTS_PER_GROUP) {
                throw new IllegalStateException("Cargo " + name + " possui pais demais (maximo " + MAX_PARENTS_PER_GROUP + ").");
            }
            List<String> permissions = normalizePermissions(section.getStringList(rawName + ".permissions"));
            if (permissions.size() > MAX_PERMISSIONS_PER_GROUP) {
                throw new IllegalStateException("Cargo " + name + " possui permissoes demais (maximo " + MAX_PERMISSIONS_PER_GROUP + ").");
            }
        ...
        for (String group : hierarchy) {
            if (!groups.containsKey(group)) {
                throw new IllegalStateException("Cargo da hierarquia nao existe em groups: " + group);
            }
        }
        for (Group group : groups.values()) {
            for (String parent : group.parents()) {
                if (!groups.containsKey(parent)) {
                    throw new IllegalStateException("Cargo " + group.name() + " referencia pai inexistente: " + parent);
                }
            }
        }
    ...
    private static List<String> normalizeGroups(List<String> values) {
        ...
        for (String value : values) {
            String normalized = normalize(value);
            if (!isSafeGroupName(normalized)) {
                throw new IllegalStateException("Nome de pai invalido: " + value);
            }
            if (!result.add(normalized)) {
                throw new IllegalStateException("Pai duplicado na configuracao: " + normalized);
            }
        }
    ...
    private static List<String> normalizePermissions(List<String> values) {
        ...
        for (String value : values) {
            ...
            if (normalized.length() > 128 || !normalized.matches("[a-z0-9_.:*\\-]+")) {
                throw new IllegalStateException("Permissao invalida: " + value);
            }
            if (normalized.contains("*")) {
                throw new IllegalStateException("Permissao excessivamente ampla nao permitida: " + value);
            }
            if (!result.add(normalized)) {
                throw new IllegalStateException("Permissao duplicada: " + normalized);
            }
        }
        return List.copyOf(result);
    }

    private void validateParentCycles() {
        Map<String, VisitState> states = new HashMap<>();
        for (String group : groups.keySet()) states.put(group, VisitState.UNVISITED);
        for (String group : groups.keySet()) detectCycle(group, states, new ArrayDeque<>());
    }

    private void detectCycle(String name, Map<String, VisitState> states, Deque<String> path) {
        VisitState state = states.get(name);
        if (state == VisitState.VISITING) {
            path.addLast(name);
            throw new IllegalStateException("Ciclo detectado na hierarquia de cargos: " + String.join(" -> ", path));
        }
        if (state == VisitState.VISITED) return;

        states.put(name, VisitState.VISITING);
        path.addLast(name);
        for (String parent : groups.get(name).parents()) detectCycle(parent, states, path);
        path.removeLast();
