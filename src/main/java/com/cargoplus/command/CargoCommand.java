...
public final class CargoCommand implements CommandExecutor, TabCompleter {
    ...
    private boolean promote(CommandSender sender, String[] args) {
        ...
        if (plugin.groups().indexOf(current) < 1) {
            sender.sendMessage(msg("not-staff"));
            return true;
        }
    ...
    private boolean setCargo(CommandSender sender, String[] args) {
        ...
        var group = plugin.groups().get(args[2]);
        if (group == null) {
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if ((args[0].equalsIgnoreCase("promover") || args[0].equalsIgnoreCase("removercargo")) && args.length == 2) {
            return onlinePlayers(args[1]);
        }
        if (args[0].equalsIgnoreCase("setcargo") && args.length == 2) return onlinePlayers(args[1]);
        if (args[0].equalsIgnoreCase("setcargo") && args.length == 3) return partial(plugin.groups().hierarchy(), args[2]);
        return List.of();
    }
    ...
    private List<String> partial(Collection<String> values, String input) {
        return values.stream().filter(v -> v.regionMatches(true, 0, input, 0, input.length()))
